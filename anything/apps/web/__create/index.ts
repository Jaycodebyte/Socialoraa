...
const adapter = process.env.DATABASE_URL ? NeonAdapter(pool) : createMemoryAdapter();
...
for (const method of ['post', 'put', 'patch'] as const) {
  app[method]('*', async (c, next) => {
    const isYouTubePublish = c.req.path.startsWith('/api/publish/youtube');
    const isScheduledMediaUpload = c.req.path.startsWith('/api/media/upload');
    const limit =
      isYouTubePublish || isScheduledMediaUpload
        ? 512 * 1024 * 1024
        : 50 * 1024 * 1024;
    ...
    return bodyLimit({
      ...
      onError: (context) => {
        ...
        return context.json(
          {
            error: `Upload is too large. Maximum allowed size for this action is ${mb} MB.`,
          },
          413
        );
      },
...
if (process.env.AUTH_SECRET) {
  const isSecureAuthUrl = process.env.AUTH_URL?.startsWith('https') ?? false;
  app.use(
    '*',
    initAuthConfig((c) => ({
